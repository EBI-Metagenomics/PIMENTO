...
def get_read_count(read_path: Path, file_type: str = "fastq") -> int:
    ...
    :type read_path: Path
    :param file_type: The type of the file, either "fastq" or "fasta". Defaults to "fastq".
    :type file_type: str
...
def compute_windowed_base_conservation(
    ...
    :param cons_list: List of base conservation dictionaries for each position.
    :type cons_list: list
    :param read_count: Total number of reads in the FASTQ file.
    :type read_count: int
    :param cons_threshold: Threshold for consensus base calling. Defaults to CONSENSUS_BASE_THRESHOLD from
        thresholds.py.
    :type cons_threshold: float
    :param do_not_include: List of positions to exclude from consensus sequence generation. Defaults to empty list.
    :type do_not_include: list
    :param counter_start: Starting index for position counter. Defaults to 1.
    :type counter_start: int
    :param max_line_count: Maximum number of reads to consider. If 0, uses read_count. Defaults to 0.
    :type max_line_count: int
    :return: Tuple containing consensus sequence string and list of conservation values.
...
def build_list_of_base_counts(
    ...
    :param read_substring_count_dict: Dictionary mapping substrings to their occurrence counts
        from fetch_read_substrings().
    :type read_substring_count_dict: dict
    :param prefix_len: Length of the prefix/substring.
    :type prefix_len: int
    :return: List of dictionaries containing base counts for each position in the substring.
    :rtype: list[defaultdict[int]]
    """
...
def fetch_read_substrings(
    ...
    :param prefix_len: Length of the prefix/substring to extract from each read.
    :type prefix_len: int
    :param rev: Whether to reverse the read sequence before extraction. Defaults to False.
    :type rev: bool
    :param start: Starting position (1-indexed) for substring extraction. Defaults to 1.
    :type start: int
    :param max_line_count: Maximum number of reads to process. If 0, processes all reads. Defaults to 0.
    :type max_line_count: int
    :return: Dictionary mapping substring sequences to their occurrence counts.
    :rtype: dict
    """
